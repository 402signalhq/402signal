...
import { canonical, digest } from './base-batch-ledger.js';
import { BASE_BATCH, BASE_COLLECTOR, BASE_USDC, readBatchState, observeBaseBatch } from './base-batch-observer.js';
import { executeBatchOperation, prepareBatchOperation } from './batch-operation-runner.js';
const freeze = (x) => { if (x && typeof x === 'object') {
    Object.values(x).forEach(freeze);
    Object.freeze(x);
} return x; };
const check = (b, m) => { if (!b)
    throw Error(m); };
const uint = (x) => { check(typeof x === 'string' && /^[1-9][0-9]{0,38}$/.test(x) && BigInt(x) < 2n ** 128n, 'invalid bounded amount'); return BigInt(x); };
...
export class BaseBatchController {
    ...
    plan;
    requirements;
    constructor(ledger, journal, rpc, plan) {
        this.ledger = ledger;
        this.journal = journal;
        this.rpc = rpc;
        this.plan = freeze(JSON.parse(canonical(plan)));
        const p = this.plan, c = p.config;
        ...
        check(u.protocol === 'https:' && !u.username && !u.password && !u.hash, 'resource refused');
        check(Number.isSafeInteger(p.expiresAt) && p.expiresAt > 0, 'campaign expiry refused');
        if (p.deliveryObservationUntil !== undefined)
            check(Number.isSafeInteger(p.deliveryObservationUntil) && p.deliveryObservationUntil >= p.expiresAt && p.deliveryObservationUntil <= Date.now() + 86400000, 'delivery observation bound refused');
    ...
    fresh() { check(Date.now() < this.plan.expiresAt, 'campaign expired; reconcile or close only'); }
    async initialize() {
        await this.ledger.initialize();
        await this.journal.initialize();
        await this.ledger.bind(this.plan);
        if (await this.ledger.get('baseline'))
            return;
        ...
        check(['balance', 'claimed', 'withdrawAmount', 'withdrawAt', 'refundNonce'].every(k => baseline[k] === '0'), 'fresh channel required');
        ...
        await this.ledger.once('baseline', baseline);
        await this.ledger.once('progress', { state: 'new' });
    }
    ...
    async bindDeliveryObservation(value) {
        const p = this.plan;
        check(p.deliveryObservationUntil !== undefined, 'second observation not enabled');
        ...
        if (prior) {
            check(canonical(prior) === canonical(value), 'delivery observation immutable conflict');
            return;
        }
        check((await this.ledger.require('progress')).state === 'active:0', 'observation requires confirmed unused deposit');
        for (let i = 1; i <= p.maxCalls; i++)
            for (const suffix of ['', ':typed:1', ':unknown', ':accepted'])
                check(!await this.ledger.get('delivery:' + i + suffix), 'delivery already attempted');
        await this.ledger.once('delivery:observation', value);
        check(canonical(await this.ledger.require('delivery:observation')) === canonical(value), 'delivery observation immutable conflict');
    }
    async deliveryFresh() {
        if (this.plan.deliveryObservationUntil === undefined) {
            this.fresh();
            return;
        }
    ...
    account(owner, stage, cap, deposit = false) {
        check(eq(owner.address, this.plan.config.payer), 'wrong owner');
        let calls = 0;
        const bound = owner.signTypedData.bind(owner), p = this.plan, id = computeChannelId(p.config, 'eip155:8453');
        return { address: owner.address, signTypedData: async (data) => {
                if (deposit)
                    this.fresh();
                else
                    await this.deliveryFresh();
                calls++;
                const expected = deposit && calls === 1 ? 'ReceiveWithAuthorization' : 'Voucher';
                check(data.primaryType === expected && calls <= (deposit ? 2 : 1), 'unexpected signing effect');
                if (expected === 'Voucher')
                    check(canonical(data.domain) === canonical({ ...BATCH_SETTLEMENT_DOMAIN, chainId: 8453, verifyingContract: BASE_BATCH }) && canonical(data.types) === canonical(voucherTypes) && data.message.channelId === id && data.message.maxClaimableAmount === BigInt(cap), 'voucher bound mismatch');
                else {
                    const m = data.message;
                    check(canonical(data.domain) === canonical({ name: 'USD Coin', version: '2', chainId: 8453, verifyingContract: BASE_USDC }) && canonical(data.types) === canonical({ ReceiveWithAuthorization: authorizationTypes.TransferWithAuthorization }) && eq(m.from, p.config.payer) && eq(m.to, BASE_COLLECTOR) && m.value === BigInt(p.depositAtomic) && m.validAfter === 0n && m.validBefore > BigInt(Date.now() / 1000 | 0) && m.validBefore <= BigInt((Date.now() / 1000 | 0) + 300) && /^0x[0-9a-f]{64}$/.test(m.nonce), 'deposit bound mismatch');
                }
                check(await this.ledger.once(stage + ':typed:' + calls, data), 'typed intent already claimed');
                return bound(data);
            } };
    }
    async prepareDeposit(owner) {
        this.fresh();
        await this.ledger.transition('new', 'deposit-signing');
        ...
        const payload = { ...result, accepted: this.requirements, resource: { url: p.resource } };
        const raw = payload.payload, auth = raw.deposit.authorization.erc3009Authorization, typed = await this.ledger.require('deposit:typed:1');
        ...
        check(await this.ledger.once('deposit:payload', payload), 'deposit already prepared');
        await this.ledger.transition('deposit-signing', 'deposit-ready');
        return payload;
    }
    async sendDeposit(provider) {
        this.fresh();
        await this.ledger.transition('deposit-ready', 'deposit-inflight');
        const payload = freeze(await this.ledger.require('deposit:payload'));
        try {
            const verified = await provider.verify(payload, this.requirements);
            check(verified.isValid, 'deposit verification failed');
            const result = await provider.settle(payload, this.requirements);
            await this.recordAck('deposit', result);
            return result;
        }
        catch {
            await this.ledger.once('deposit:unknown', { state: 'unknown' });
            return { state: 'unknown', newPaymentAllowed: false };
        }
    }
    async recordAck(stage, result) { check(result.success && result.network === 'eip155:8453' && /^0x[0-9a-fA-F]{64}$/.test(result.transaction), 'provider outcome unknown'); await this.ledger.once(stage + ':ack', { transactionHash: result.transaction }); }
    async confirm(kind, transactionHash) {
        const prior = await this.ledger.get(kind + ':confirmed');
        if (prior) {
            ...
            if (current === kind + '-inflight')
                await this.ledger.transition(current, next);
            return prior;
        }
        check((await this.ledger.require('progress')).state === kind + '-inflight', 'operation was not sent');
        const ack = await this.ledger.get(kind + ':ack'), hash = transactionHash ?? ack?.transactionHash;
        check(hash, 'existing transaction hash required');
        ...
        const effect = { kind, config: p.config, amount, transactionHash: hash, payload, baseline, maxBuyerGasWei: p.maximumBuyerGasWei };
        const observed = await observeBaseBatch(this.rpc, effect);
        if (observed.state !== 'chain_confirmed')
            return observed;
        if (kind === 'deposit')
            observed.confirmedAtSeconds = Math.floor(Date.now() / 1000);
        if (kind === 'claim' || kind === 'settle') {
            ...
            const evidence = { status: 'chain_confirmed', transactionHash: hash.toLowerCase(), blockHash: observed.blockHash.toLowerCase(), blockNumber: observed.blockNumber, evidenceDigest: observed.evidenceDigest };
            if (existing?.state === 'chain_confirmed')
                check(last?.transactionHash === evidence.transactionHash && last?.blockHash === evidence.blockHash && last?.blockNumber === evidence.blockNumber && last?.evidenceDigest === evidence.evidenceDigest, 'journal confirmation conflicts');
            else
                await this.journal.reconcile(operationId, evidence);
        }
        ...
        const current = await this.ledger.require('progress');
        if (current.state !== next[1])
            await this.ledger.transition(next[0], next[1]);
        return observed;
    }
    async deliver(owner, sequence, requestDigest, send) {
        await this.deliveryFresh();
        check(Number.isInteger(sequence) && sequence >= 1 && sequence <= this.plan.maxCalls && /^[0-9a-f]{64}$/.test(requestDigest), 'delivery bounds refused');
        ...
        let voucher;
        if (sequence === 1)
            voucher = (await this.ledger.require('deposit:payload')).payload.voucher;
        else
            voucher = await signVoucher(this.account(owner, 'delivery:' + sequence, cap), computeChannelId(this.plan.config, 'eip155:8453'), cap, 'eip155:8453');
        const payload = { x402Version: 2, accepted: this.requirements, resource: { url: this.plan.resource }, payload: { type: 'voucher', channelConfig: this.plan.config, voucher } };
        check(await this.ledger.once('delivery:' + sequence, { requestDigest, payload, cap }), 'delivery already exists');
        try {
            await this.deliveryFresh();
            const result = await send(payload);
            check(result.chargedAmount === this.plan.perCallAtomic && result.chargedCumulativeAmount === cap, 'unconfirmed merchant accounting');
            await this.ledger.once('delivery:' + sequence + ':accepted', { cap });
            await this.ledger.transition('delivery-inflight:' + sequence, 'active:' + sequence);
            return { state: 'voucher_accepted', chainSettled: false };
        }
        catch {
            await this.ledger.once('delivery:' + sequence + ':unknown', { state: 'unknown' });
            return { state: 'unknown', newPaymentAllowed: false };
        }
    }
    ...
    async recoverDelivery(sequence, recover) {
        ...
        if (prior) {
            check(prior.cap === cap, 'accepted delivery mismatch');
            if (state === 'delivery-inflight:' + sequence)
                await this.ledger.transition(state, 'active:' + sequence);
            return { state: 'voucher_accepted', chainSettled: false };
        }
        ...
        let attempt;
        for (let n = 1; n <= 6; n++)
            if (await this.ledger.once(stage + ':recovery:' + n, { authorizationDigest: sha(authorization) })) {
                attempt = stage + ':recovery:' + n;
                break;
            }
        ...
        try {
            ...
            await this.ledger.once(attempt + ':evidence', result);
            await this.ledger.once(stage + ':accepted', { cap });
            const current = (await this.ledger.require('progress')).state;
            if (current === 'delivery-inflight:' + sequence)
                await this.ledger.transition(current, 'active:' + sequence);
    ...
    async close(sequence) { check(sequence >= 1 && sequence <= this.plan.maxCalls && Number.isInteger(sequence), 'invalid close sequence'); await this.ledger.transition('active:' + sequence, 'closing'); const d = await this.ledger.require('delivery:' + sequence); await this.ledger.once('closing', { charged: d.cap, voucher: d.payload.payload.voucher }); }
    async operation(kind) {
        ...
        return { cycleId: this.ledger.campaignId, scope: { network: 'eip155:8453', receiver: p.config.receiver, token: p.config.token }, requirements: r, paymentPayload: { x402Version: 2, accepted: r, payload } };
    }
    async sendCloseOperation(kind, provider) {
        await this.ledger.transition(kind === 'claim' ? 'closing' : 'claimed', kind + '-inflight');
        const x = await this.operation(kind);
        await this.ledger.once(kind + ':payload', x.paymentPayload);
        const result = await executeBatchOperation(this.journal, provider, x);
        const event = result.operation?.events.at(-1);
        if (event?.transactionHash)
            await this.ledger.once(kind + ':ack', { transactionHash: event.transactionHash });
        return result;
    }
    async sendRefund(provider) {
        const closing = await this.ledger.require('closing'), after = (await this.ledger.require('settle:confirmed')).after;
        const amount = BigInt(this.plan.depositAtomic) - BigInt(closing.charged);
        check(amount > 0n, 'no refundable remainder; use closeEmpty');
        await this.ledger.transition('paid', 'refund-inflight');
        const r = { ...this.requirements, amount: '0', maxTimeoutSeconds: 0 };
        const paymentPayload = { x402Version: 2, accepted: r, payload: { type: 'refund', channelConfig: this.plan.config, voucher: closing.voucher, amount: amount.toString(), refundNonce: after.refundNonce, claims: [] } };
        await this.ledger.once('refund:payload', paymentPayload);
        try {
            const result = await provider.settle(paymentPayload, r);
            await this.recordAck('refund', result);
            return result;
        }
        catch {
            return { state: 'unknown', newPaymentAllowed: false };
    ...
    async sendUnspentRefund(provider) {
        ...
        const paymentPayload = { x402Version: 2, accepted: requirements, payload: { type: 'refund', channelConfig: this.plan.config, voucher, amount: this.plan.depositAtomic, refundNonce: current.refundNonce, claims: [] } };
        check(await this.ledger.once('refund:payload', paymentPayload), 'refund payload already exists');
        try {
            const result = await provider.settle(paymentPayload, requirements);
            await this.recordAck('refund', result);
            return result;
        }
        catch {
...
//# sourceMappingURL=base-batch-lifecycle.js.map