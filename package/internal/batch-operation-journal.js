...
function identifier(value) {
    if (typeof value !== "string" || !DIGEST.test(value))
        throw new Error("invalid operation id or digest");
}
function canonicalPlan(input) {
    identifier(input.operationId);
    identifier(input.payloadDigest);
    if (input.kind !== "claim" && input.kind !== "settle")
        throw new Error("invalid operation kind");
    const scope = input.scope;
    if (!scope || typeof scope.network !== "string" || !/^eip155:[1-9][0-9]{0,19}$/.test(scope.network) ||
        ![scope.receiver, scope.token].every(v => typeof v === "string" && /^0x[0-9a-fA-F]{40}$/.test(v)))
...
function event(state, proof = {}) {
    return { state, at: new Date().toISOString(), ...proof };
}
function evidence(input) {
    identifier(input.evidenceDigest);
    if (input.transactionHash !== undefined && !HASH.test(input.transactionHash))
        throw new Error("invalid transaction hash");
    const result = { evidenceDigest: input.evidenceDigest };
    ...
    if (input.status === "chain_confirmed") {
        if (!input.transactionHash || typeof input.blockHash !== "string" || !HASH.test(input.blockHash) ||
            ...
            throw new Error("chain observation requires transaction and block evidence");
        result.blockHash = input.blockHash;
        result.blockNumber = input.blockNumber;
    }
    return result;
}
function sameEvent(current, state, proof) {
    const last = current.events.at(-1);
    if (!last || current.state !== state)
        return false;
    return ["evidenceDigest", "transactionHash", "blockHash", "blockNumber"]
        .every(key => last[key] === proof[key]);
}
function decode(raw, expectedId) {
    if (!raw || typeof raw !== "object" || Buffer.byteLength(JSON.stringify(raw)) > 32768)
        throw new Error("invalid operation record");
    const value = structuredClone(raw);
    canonicalPlan(value);
    if (expectedId !== undefined && value.operationId !== expectedId)
        throw new Error("operation record id mismatch");
    ...
    if (states[0] !== "planned" || (states.length > 1 && states[1] !== "inflight") ||
        ...
        value.events.some(entry => typeof entry.at !== "string" || !Number.isFinite(Date.parse(entry.at))))
        throw new Error("invalid operation history");
    for (const entry of value.events.slice(2)) {
        identifier(entry.evidenceDigest);
        if (entry.transactionHash !== undefined && !HASH.test(entry.transactionHash))
            throw new Error("invalid recorded transaction hash");
        if (entry.state === "chain_confirmed" && (!entry.transactionHash || !entry.blockHash || !HASH.test(entry.blockHash) ||
            !Number.isSafeInteger(entry.blockNumber) || entry.blockNumber < 0))
            throw new Error("invalid recorded chain observation");
    }
    return value;
}
...
export class PostgresBatchOperationJournal {
    ...
    async plan(input) {
        ...
        const value = { ...plan, state: "planned", events: [event("planned")] };
      ...
      ON CONFLICT(namespace,operation_id) DO NOTHING`, [this.namespace, plan.operationId, JSON.stringify(value)]);
        ...
        if (!existing || JSON.stringify(canonicalPlan(existing)) !== JSON.stringify(plan))
            throw new Error("operation id conflicts with immutable plan");
        return existing;
    }
    ...
    async acquire(operationId) {
        ...
        return this.transaction(operationId, async (client, current) => {
            ...
            const next = { ...current, sendToken, state: "inflight", events: [...current.events, event("inflight")] };
            await this.save(client, next);
            return { ...canonicalPlan(current), sendToken };
        });
    }
    async recordOutcome(operationId, sendToken, input) {
        if (!["provider_ack", "failed", "unknown"].includes(input.status))
            throw new Error("invalid provider outcome");
        const proof = evidence(input);
        return this.transaction(operationId, async (client, current) => {
            if (current.sendToken !== sendToken || typeof sendToken !== "string")
                throw new Error("send token does not own operation");
            if (sameEvent(current, input.status, proof))
                return exposed(current);
            if (current.state !== "inflight")
                throw new Error("operation outcome already recorded; reconcile explicitly");
            const next = { ...current, state: input.status, events: [...current.events, event(input.status, proof)] };
            await this.save(client, next);
            return exposed(next);
        });
    }
    async reconcile(operationId, input) {
        if (input.status !== "chain_confirmed" && input.status !== "not_executed")
            throw new Error("invalid reconciliation outcome");
        const proof = evidence(input);
        return this.transaction(operationId, async (client, current) => {
            if (sameEvent(current, input.status, proof))
                return exposed(current);
            if (["planned", "chain_confirmed", "not_executed"].includes(current.state))
                throw new Error("operation cannot accept this reconciliation");
            const next = { ...current, state: input.status, events: [...current.events, event(input.status, proof)] };
            await this.save(client, next);
            return exposed(next);
        });
    }
    async save(client, value) {
        decode(value);
        await client.query(`UPDATE ${TABLE} SET record=$3::jsonb WHERE namespace=$1 AND operation_id=$2`, [this.namespace, value.operationId, JSON.stringify(value)]);
    }
    async transaction(operationId, action) {
        identifier(operationId);
        ...
        try {
            ...
            if (!result.rows[0])
                throw new Error("operation has not been planned");
...
//# sourceMappingURL=batch-operation-journal.js.map