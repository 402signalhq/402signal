import { parseAbi, decodeFunctionData, decodeEventLog, encodeFunctionData, decodeFunctionResult, keccak256, encodeAbiParameters } from 'viem';
...
const must = (yes) => { if (!yes)
    throw Error('unconfirmed effect'); };
const hash = (s) => typeof s === 'string' && /^0x[0-9a-fA-F]{64}$/.test(s);
export async function readBatchState(rpc, config, block) {
    const id = computeChannelId(config, 'eip155:8453');
    const read = async (functionName, args) => { const data = encodeFunctionData({ abi: BASE_BATCH_ABI, functionName, args }); const raw = await rpc('eth_call', [{ to: BASE_BATCH, data }, block]); return decodeFunctionResult({ abi: BASE_BATCH_ABI, functionName, data: raw }); };
 ...
 */
export async function observeBaseBatch(rpc, e) {
    try {
        ...
        must(height > 0n && BigInt(finalized.number) >= height && hash(finalized.hash));
        const block = await rpc('eth_getBlockByNumber', [receipt.blockNumber, false]);
        must(eq(block.hash, receipt.blockHash));
        must(BigInt(tx.value ?? '0x0') === 0n && eq(tx.from, receipt.from) && height <= BigInt(Number.MAX_SAFE_INTEGER));
        if (eq(tx.from, e.config.payer))
            must(BigInt(receipt.gasUsed) * BigInt(receipt.effectiveGasPrice) <= BigInt(e.maxBuyerGasWei));
        const decoded = decodeFunctionData({ abi: BASE_BATCH_ABI, data: tx.input }), args = decoded.args;
        const cfg = (c) => computeChannelId(c, 'eip155:8453') === computeChannelId(e.config, 'eip155:8453');
        const n = BigInt(e.amount), p = e.payload;
        if (e.kind === 'deposit')
            must(decoded.functionName === 'deposit' && cfg(args[0]) && args[1] === n && eq(args[2], BASE_COLLECTOR));
        if (e.kind === 'claim')
            must(decoded.functionName === 'claimWithSignature' && args[0].length === 1 && cfg(args[0][0].voucher.channel) && args[0][0].totalClaimed === n && args[0][0].voucher.maxClaimableAmount === BigInt(p.claims[0].voucher.maxClaimableAmount) && eq(args[0][0].signature, p.claims[0].signature));
        if (e.kind === 'settle')
            must(decoded.functionName === 'settle' && eq(args[0], e.config.receiver) && eq(args[1], e.config.token));
        if (e.kind === 'refund')
            must(decoded.functionName === 'refundWithSignature' && cfg(args[0]) && args[1] === n && args[2] === BigInt(e.baseline.refundNonce));
        must(n > 0n && n < 2n ** 128n);
        const before = await readBatchState(rpc, e.config, ('0x' + (height - 1n).toString(16))), after = await readBatchState(rpc, e.config, receipt.blockNumber);
        for (const key of ['balance', 'claimed', 'receiverClaimed', 'receiverSettled', 'refundNonce'])
            must(before[key] === e.baseline[key]);
        must(before.withdrawAmount === '0' && after.withdrawAmount === '0');
        const delta = (key) => BigInt(after[key]) - BigInt(before[key]);
        ...
        const net = new Map();
        let used = false, settled = false;
        must(Array.isArray(receipt.logs) && receipt.logs.length <= 128);
        for (const log of receipt.logs) {
            if (eq(log.address, BASE_USDC)) {
                try {
                    const event = decodeEventLog({ abi: TOKEN_ABI, data: log.data, topics: log.topics });
                    const a = event.args;
                    if (event.eventName === 'Transfer') {
                        const f = a.from.toLowerCase(), t = a.to.toLowerCase();
                        net.set(f, (net.get(f) ?? 0n) - a.value);
                        net.set(t, (net.get(t) ?? 0n) + a.value);
                    }
                    if (event.eventName === 'AuthorizationUsed' && e.kind === 'deposit') {
                        const auth = p.deposit.authorization.erc3009Authorization;
                        const nonce = keccak256(encodeAbiParameters([{ type: 'bytes32' }, { type: 'uint256' }], [computeChannelId(e.config, 'eip155:8453'), BigInt(auth.salt)]));
                        if (eq(a.authorizer, e.config.payer) && eq(a.nonce, nonce))
                            used = true;
                    }
                }
                catch { /* Other token events cannot substitute for required effects. */ }
            }
            if (eq(log.address, BASE_BATCH) && e.kind === 'settle')
                try {
                    const event = decodeEventLog({ abi: BASE_BATCH_ABI, data: log.data, topics: log.topics });
                    const a = event.args;
                    if (event.eventName === 'Settled' && eq(a.receiver, e.config.receiver) && eq(a.token, e.config.token) && a.amount === n)
                        settled = true;
                }
                catch { }
        }
        const value = (addr) => net.get(addr.toLowerCase()) ?? 0n;
        ...
        must(eq(canonicalBlock.hash, receipt.blockHash));
...
//# sourceMappingURL=base-batch-observer.js.map