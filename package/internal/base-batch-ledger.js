...
export class BaseBatchLedger {
    ...
    async initialize() {
        await this.pool.query(`CREATE TABLE IF NOT EXISTS ${TABLE}(campaign text NOT NULL,stage text NOT NULL,value jsonb NOT NULL CHECK(octet_length(value::text)<=65536),PRIMARY KEY(campaign,stage))`);
    }
    async get(stage) {
        const r = await this.pool.query(`SELECT value FROM ${TABLE} WHERE campaign=$1 AND stage=$2`, [this.campaignId, stage]);
        return r.rows[0]?.value;
    }
    async once(stage, value) {
        if (!/^[a-zA-Z0-9:_-]{1,100}$/.test(stage))
            throw Error("invalid stage");
        const raw = canonical(value);
        if (Buffer.byteLength(raw) > 65536)
            throw Error("stage too large");
        const r = await this.pool.query(`INSERT INTO ${TABLE}(campaign,stage,value) VALUES($1,$2,$3::jsonb) ON CONFLICT DO NOTHING RETURNING stage`, [this.campaignId, stage, raw]);
        return r.rowCount === 1;
    }
    async transition(expected, next) {
        const r = await this.pool.query(`UPDATE ${TABLE} SET value=$3::jsonb WHERE campaign=$1 AND stage='progress' AND value->>'state'=$2 RETURNING stage`, [this.campaignId, expected, JSON.stringify({ state: next })]);
        if (r.rowCount !== 1)
    ...
    async require(stage) {
        ...
        if (!v)
            throw Error("stage required: " + stage);
        return v;
    }
}
//# sourceMappingURL=base-batch-ledger.js.map