...
- `402signal` (Python) 0.1.2 candidate (security review follow-up): the
  client refuses redirects (`signal402.RedirectRefused`; the router never
  redirects, and following one would have forwarded the payment header to
  whoever answered), validates the router URL (https or loopback http, no
  credentials, query or fragment) and bounds every response read to 256 KiB
  (`MAX_RESPONSE_BYTES`). Published from tag `python-v0.1.2` after the train.
