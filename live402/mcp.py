...
ROUTE_DESCRIPTION = (
    ...
    "routing evidence. Does not buy the seller's service. Seller payment is separate; the agent "
    "keeps the wallet.\n\n"
    "Use for fresh probing and constraint-based selection, or signed binding evidence before "
    "merchant sign. Use preview for free catalog discovery without probing. Use validate for an "
    "unpaid readiness check of one catalog-listed URL.\n\n"
    "Pass need (capability) and/or url (HTTPS). If both are set, url selects the endpoint; need "
    "may add policy context. First unsigned call returns HTTP 402 for the $0.003 USDC routing "
    "fee. Payment needs an x402-capable HTTP client (not a wallet key argument). Credential-free "
    "Glama stdio cannot complete paid route. Fee settles only on a qualifying live result. A "
    "completed miss means no eligible live route under the submitted rules and is not settled. "
    "Settled fees are not reversed if the seller offer later expires or changes. On unknown "
    "billing, stop and reconcile; do not create another authorization. MCP has no recovery-only "
    "path.\n\n"
    "Beyond the schema: networks is a hard allowlist (omit = all rails; [] = none); "
    "prefer_network only ranks. Structured constraints override prose in policy/need; inspect "
    "unresolved_constraints. max_price_usd / max_total_cost_usd bound the seller, not the "
...
PREVIEW_DESCRIPTION = (
    ...
    "sellers. Returns claimed listings and any earlier observations; not_probed=true means this "
    "is not a new live check. Freshness is for this search response, not each seller's last "
    "probe. Results can be limited or non-exhaustive.\n\n"
    "Use first to find candidates by need. Use validate next for an unpaid readiness check of "
    "one concrete catalog-listed URL. Use route when you need fresh probing, constraint-based "
    "selection, or signed routing evidence.\n\n"
    "Pass a nonblank need (capability), not a URL. networks is a hard allowlist (omit = all "
    "supported rails; [] = none). prefer_network only ranks and cannot add an excluded rail. A "
    "chain named in need can influence ranking when prefer_network is omitted. Preview does not "
    "enforce a purchase budget, reserve a price, or authorize payment."
)
...
VALIDATE_DESCRIPTION = (
    ...
    "catalog (for example a URL from preview). Compares claimed vs observed payment/readiness "
    "flags. Does not buy the service or append a routing observation to history.\n\n"
    "Use for a free readiness check of a known catalog URL. Use preview instead to find "
    "endpoints by capability. Use route for live constraint-based selection, signed routing "
