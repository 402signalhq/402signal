...
Published client and guard: npm install @402signal/route-guard@0.7.4 (then npm audit signatures for the provenance attestation), or the digest-pinned archive at https://github.com/402signalhq/402signal/releases/tag/route-guard-v0.7.4
One-command verified archive install from a reviewed checkout: node scripts/install_route_guard.mjs
It downloads the published GitHub archive, checks both /capabilities.json pins, then npm install --ignore-scripts. Without a checkout, download 402signal-route-guard-0.7.4.tgz and SHA256SUMS from that release, run sha256sum --check SHA256SUMS, then npm install the matching archive. Node.js >=22 is required. Package exports include @402signal/route-guard, /client, /file-store, /recovery, the separate /batch guard, the /x402 hook and the /mpp hook. The default wrap is wrapExactAuthorize in exact-authorize.mjs (written by the installer): observe, bind, locally verify, then the existing wallet. Same wrap on the next spend. Fail closed. The packaged examples/search.ts is the longer pay-fetch form. Hosted hop is session=hop plus session_id; a raw id in session is miss_reason=invalid_session_shape, HTTP 200, no checking fee. wrapExactAuthorize already refuses hops. Full API: https://github.com/402signalhq/402signal/tree/main/sdk/route-guard
