...
The intended chk_grp verifier package is route-guard-v0.7.2; it stays pending until that GitHub release exists. Verify the published digest, then npm install the matching archive. Node.js >=22 is required. This is a release archive, not an npm registry publication. Package exports include @402signal/route-guard, /client, /file-store, /recovery and the separate /batch guard. Full API: https://github.com/402signalhq/402signal/tree/main/sdk/route-guard
