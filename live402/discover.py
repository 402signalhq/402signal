...
GUIDANCE = (
    ...
    "a bazaar HTTP GET that advertises empty queryParams (or an empty-object queryParams schema) and no required body is the same signal; "
...
It downloads the published GitHub archive, checks both /capabilities.json pins, then npm install --ignore-scripts. Without a checkout, download 402signal-route-guard-0.7.2.tgz and SHA256SUMS from that release, run sha256sum --check SHA256SUMS, then npm install the matching archive. Node.js >=22 is required. This is a release archive, not an npm registry publication. Package exports include @402signal/route-guard, /client, /file-store, /recovery, the separate /batch guard and the /x402 hook. The default wrap is wrapExactAuthorize in exact-authorize.mjs (written by the installer): observe, bind, locally verify, then the existing wallet. Same wrap on the next spend. Fail closed. The packaged examples/search.ts is the longer pay-fetch form. Hosted hop is session=hop plus session_id; a raw id in session is miss_reason=invalid_session_shape, HTTP 200, no routing fee. wrapExactAuthorize already refuses hops. Full API: https://github.com/402signalhq/402signal/tree/main/sdk/route-guard

Official x402 client (@x402/core with @x402/fetch): import { signalGuard } from "@402signal/route-guard/x402" and register client.onBeforePaymentCreation(signalGuard({ fetchWithPayment, trustedLogVkey })). Before each seller payment the hook pays one $0.003 check with the same wallet, re-reads the seller's raw challenge, verifies the signed receipt locally and aborts the payment when the selected terms differ from the verified terms or no qualifying offer was found. It never sees wallet keys, and payments to 402signal.com itself are not checked.
