...
test("installer copies the wrap next to the published package", (t) => {
  ...
  const publishedTgz = join(packDir, "402signal-route-guard-0.7.7.tgz");
  assert.ok(existsSync(publishedTgz), "PACK_DIR must hold the published 0.7.7 installer pin");
  ...
  try {
    const result = spawnSync(process.execPath, [
      ...
      "--capabilities", join(root, "protocol/capabilities.json"),
    ], { encoding: "utf8" });
