...
const capabilitiesPath = join(root, "protocol/capabilities.json");
...
test("unpublished capabilities row is not an install URL", () => {
  ...
  try {
    ...
    const publishedIdx = pending.packages.findIndex((row) => row.tag === "route-guard-v0.7.7");
    ...
    writeFileSync(join(dir, "402signal-route-guard-0.7.7.tgz"), "x");
    writeFileSync(join(dir, "SHA256SUMS"), `${"00".repeat(32)}  402signal-route-guard-0.7.7.tgz\n`);
    const result = runInstaller([
      "--verify-only",
      "--archive",
      join(dir, "402signal-route-guard-0.7.7.tgz"),
      "--checksum-file",
      join(dir, "SHA256SUMS"),
      "--capabilities",
      capabilities,
...
test("reviewed local archive verifies and installs into a fresh buyer directory", (t) => {
  ...
  const tgz = join(packDir, "402signal-route-guard-0.7.7.tgz");
  assert.ok(existsSync(tgz), "PACK_DIR must hold the published 0.7.7 installer pin");
...
test("install falls back after mocked idealTree and records the tool", () => {
  ...
  const result = installVerifiedArchive({
    archive: "/tmp/402signal-route-guard-0.7.7.tgz",
