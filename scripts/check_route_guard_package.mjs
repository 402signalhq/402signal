 ...
 * CI installs the exact integration/client-runtime lock first. A cloud checkout may reuse
...
const labModules = arguments_.length ? arguments_[0].slice('--lab-node-modules='.length) : join(root, 'integration/client-runtime/node_modules');
assert(isAbsolute(labModules), 'lab node_modules path must be absolute');
const readJson = async path => JSON.parse(await fs.readFile(path, 'utf8'));
const lab = await readJson(join(root, 'integration/client-runtime/package.json'));
...
`);
  ...
  const capabilities = await readJson(join(root, 'protocol/capabilities.json'));
