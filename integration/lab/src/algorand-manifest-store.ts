...
import { assert, canonical, parseJson } from "./json.js";
...
export class AlgorandManifestStore implements AlgorandManifestJournal {
  ...
  constructor(path: string) {
    assert(
      path !== ":memory:" || process.env.LIVE402_FIXTURE === "1",
    ...
    try {
      ...
      assert(
        names.every((x) =>
          ["manifest_meta", "manifest_records"].includes(x.name),
        ),
