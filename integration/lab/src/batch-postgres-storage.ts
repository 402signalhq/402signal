...
function channelKey(value: string): string {
  if (typeof value !== "string" || !/^0x[0-9a-fA-F]{64}$/.test(value)) throw new Error("invalid channel id");
  return value.toLowerCase();
}
function uint(value: unknown): boolean {
  return typeof value === "string" && /^(0|[1-9][0-9]{0,77})$/.test(value) && BigInt(value) <= UINT256;
}
...
function record(value: unknown, id: string): Channel {
  ...
  if (Buffer.byteLength(raw) > MAX_RECORD_BYTES) throw new Error("channel record too large");
  ...
  if (!cfg || ![cfg.payer, cfg.payerAuthorizer, cfg.receiver, cfg.receiverAuthorizer, cfg.token]
    .every(x => typeof x === "string" && /^0x[0-9a-fA-F]{40}$/.test(x)) ||
    typeof cfg.salt !== "string" || !/^0x[0-9a-fA-F]{64}$/.test(cfg.salt) || !timestamp(cfg.withdrawDelay)) {
    throw new Error("invalid channel configuration");
  }
  if (![c.chargedCumulativeAmount, c.signedMaxClaimable, c.balance, c.totalClaimed].every(uint) ||
    ![c.withdrawRequestedAt, c.refundNonce, c.lastRequestTimestamp].every(timestamp) ||
    (c.onchainSyncedAt !== undefined && !timestamp(c.onchainSyncedAt)) ||
    typeof c.signature !== "string" || !/^(?:|0x(?:[0-9a-fA-F]{2})*)$/.test(c.signature)) {
    throw new Error("invalid channel state");
  }
  if (c.pendingRequest !== undefined) {
    const pending = c.pendingRequest;
    if (!pending || typeof pending.pendingId !== "string" || !/^[a-zA-Z0-9:_-]{1,128}$/.test(pending.pendingId) ||
      !uint(pending.signedMaxClaimable) || !timestamp(pending.expiresAt)) throw new Error("invalid pending request");
  }
  c.channelId = id;
  return c;
}
...
export class PostgresChannelStorage implements ChannelStorage {
  constructor(private readonly pool: Pool, private readonly namespace: string, private readonly maxListRows = 1000) {
    if (!/^[a-zA-Z0-9:_-]{1,128}$/.test(namespace)) throw new Error("invalid storage namespace");
    if (!Number.isSafeInteger(maxListRows) || maxListRows < 1 || maxListRows > 10000) throw new Error("invalid list bound");
  }
  ...
  async get(channelId: string): Promise<Channel | undefined> {
    ...
    const result = await this.pool.query(`SELECT record FROM ${TABLE} WHERE namespace=$1 AND channel_id=$2`, [this.namespace, id]);
  ...
  async list(): Promise<Channel[]> {
    const result = await this.pool.query(`SELECT channel_id, record FROM ${TABLE} WHERE namespace=$1 ORDER BY channel_id LIMIT $2`,
      [this.namespace, this.maxListRows + 1]);
    if (result.rows.length > this.maxListRows) throw new Error("channel list exceeds configured bound");
   ...
   */
  async updateChannel(channelId: string, update: (current: Channel | undefined) => Channel | undefined) {
    const id = channelKey(channelId);
    const lock = createHash("sha256").update(`${this.namespace}\0${id}`).digest().readBigInt64BE(0).toString();
    ...
    try {
      ...
      const result = await client.query(`SELECT record FROM ${TABLE} WHERE namespace=$1 AND channel_id=$2`, [this.namespace, id]);
      const stored = result.rows[0] ? record(result.rows[0].record, id) : undefined;
      ...
      if (next === current) {
        ...
        await client.query(`DELETE FROM ${TABLE} WHERE namespace=$1 AND channel_id=$2`, [this.namespace, id]);
        channel = undefined;
        status = stored === undefined ? "unchanged" : "deleted";
      } else {
        channel = record(next, id);
        await client.query(`INSERT INTO ${TABLE} (namespace, channel_id, record) VALUES ($1,$2,$3::jsonb)
          ON CONFLICT (namespace,channel_id) DO UPDATE SET record=EXCLUDED.record`, [this.namespace, id, JSON.stringify(channel)]);
        status = "updated";
      }
