...
import type { Ledger, Outcome } from "./ledger.js";
import type { Transport } from "./transport.js";
// @ts-expect-error The independently maintained pure guard is a native JS module.
import { validateAlgorandGenericProfile } from "../../sdk/route-guard/batch-profiles/algorand-generic.mjs";
...
export function algorandBatchRequest(url: string, origin: string) {
  const u = new URL(url),
    base = new URL(origin);
  assert(
    base.protocol === "https:" &&
      ...
      !u.hash &&
      url.length <= 4096,
    "algorand_batch_resource_refused",
  );
  ...
  assert(
    entries.length === 2 &&
      ...
      entries.filter(([k]) => k === "right").length === 1,
    "algorand_batch_input_refused",
  );
  const texts = ["left", "right"].map((k) => u.searchParams.get(k)!);
  assert(
    texts.every(
      (s) => s.length > 0 && s.length <= 1024 && !/[\uD800-\uDFFF]/u.test(s),
    ),
    "algorand_batch_input_refused",
  );
  return {
...
export function algorandBatchManifest(
  ...
  assert(
    ...
    "algorand_batch_unit_price_refused",
  );
  return {
    version: 1,
    ...
    resource: url,
    requestHash: digest(
      canonical({ url, method: "GET", body_sha256: digest("") }),
    ),
    itemCount: 2,
    itemAmount: "1000",
    totalAmount: "2000",
...
export type AlgorandBatchManifest = ReturnType<typeof algorandBatchManifest>;
export function buildAlgorandBatchTransactions(
  ...
  assert(
    typeof firstValid === "bigint" && typeof lastValid === "bigint",
    "algorand_batch_validity_refused",
  );
  ...
  checkAlgorandBatchGroup(
    ...
    buyer,
    (BigInt(requirement.amount) * 2n).toString(),
    15000n,
  );
  return raw;
}
export function prepareAlgorandBatch(input: {
  ...
  assert(
    input.profile === undefined ||
      input.profile === "algorand-atomic-two-item-v1",
    "algorand_batch_profile_refused",
  );
  assert(
    (input.profile === undefined) === (input.buyerLimits === undefined),
    "algorand_batch_limits_required",
  );
  ...
  assert(
    new URL(input.url).origin === input.origin,
    "algorand_batch_resource_refused",
  );
  ...
  const manifest: AlgorandBatchManifest = generic
    ? validateAlgorandGenericProfile(
        ...
        { url: input.url, method: "GET", body_sha256: digest("") },
        buyerLimits,
      )
    : algorandBatchManifest(input.url, input.origin, requirement);
  assert(
    canonical(manifest) === canonical(input.manifest),
    "algorand_batch_manifest_refused",
  );
  assert(
    generic || requirement.amount === "1000",
    "algorand_batch_unit_price_refused",
  );
  ...
  const group = checkAlgorandBatchGroup(
    ...
    [requirement, requirement],
    input.buyer,
    input.maxSpendAtomic,
    15000n,
  );
  assert(group.transfers.length === 2, "algorand_batch_size_refused");
  const id = digest(
    canonical(["algorand-atomic-batch-v1", requirement.network, group.groupId]),
  );
  const scope = digest(
    canonical({
      url: request.url,
  ...
  return {
    ...
    ...(generic
      ? {
          ...
          buyerLimits: buyerLimits as AlgorandGenericLimits,
        }
      : {}),
  };
}
export type AlgorandBatchPlan = ReturnType<typeof prepareAlgorandBatch>;
export async function signAlgorandBatch(
  ...
  assert(
    Array.isArray(signed) && signed.length === 3 && !signed[0],
  ...
  for (const index of [1, 2]) {
    ...
    const s = decodeSignedTransaction(bytes),
      tx = decodeTransaction(raw[index]!);
    assert(
      Buffer.from(encodeTransactionRaw(s.txn)).equals(
        Buffer.from(raw[index]!),
      ) &&
        s.sig?.length === 64 &&
        !s.msig &&
        !s.lsig &&
    ...
    assert(
      await ed25519Verifier(
        s.sig!,
        bytesForSigning.transaction(tx),
        Address.fromString(buyer).publicKey,
      ),
      "algorand_batch_signature_refused",
    );
 ...
 */
export async function executeAlgorandBatch(
  ledger: Pick<Ledger, "lookup" | "reserve" | "attempting" | "finish">,
  input: AlgorandBatchPlan,
  sign: (
    raw: Uint8Array[],
    indexes: number[],
  ) => Promise<(Uint8Array | null | undefined)[]>,
 ...
 */
export async function confirmAlgorandBatchOnce(
  input: AlgorandBatchPlan,
  rpcUrl: string,
  send: Transport,
) {
  try {
    ...
    assert(
      ...
      "algorand_batch_network_refused",
    );
    ...
    for (const raw of plan.raw) {
      ...
      assert(
        ...
        "algorand_batch_unconfirmed",
      );
      const tx = transactionCodec.decode(found.body?.txn?.txn, "json");
      assert(
        tx.txId() === id &&
          Buffer.from(encodeTransactionRaw(tx)).equals(Buffer.from(raw)),
        "algorand_batch_confirmation_mismatch",
      );
      if (round === undefined) round = n;
      else assert(round === n, "algorand_batch_round_mismatch");
    }
    return {
