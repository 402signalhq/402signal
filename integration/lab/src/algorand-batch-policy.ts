...
import { assert, canonical } from "./json.js";
const ALGO_GENESIS = "wGHE2Pwdvd7S12BL5FaOP20EGYesN73ktiC1qzkkit8=";
const atomic = (s: string) => {
  assert(
    typeof s === "string" && /^(0|[1-9][0-9]{0,19})$/.test(s),
    "batch_invalid_atomic",
  );
  return BigInt(s);
};
...
export function checkAlgorandBatchGroup(
  raw: Uint8Array[],
  signerIndexes: number[],
  requirements: PaymentRequirements[],
  buyer: string,
  maxSpendAtomic: string,
  maxSponsorFeeMicroAlgo: bigint,
) {
  assert(
    requirements.length >= 1 &&
      requirements.length <= 15 &&
      raw.length === requirements.length + 1,
    "algorand_batch_size_refused",
  );
  assert(
    canonical(signerIndexes) === canonical(requirements.map((_, i) => i + 1)),
    "algorand_batch_signers_refused",
  );
  const spend = requirements.reduce((sum, r) => sum + atomic(r.amount), 0n);
  assert(
    spend > 0n && spend <= atomic(maxSpendAtomic),
    "algorand_batch_spend_cap",
  );
  const sponsor = requirements[0]!.extra?.feePayer;
  assert(
    typeof sponsor === "string" &&
      sponsor !== buyer &&
      maxSponsorFeeMicroAlgo > 0n,
    "algorand_batch_sponsor_refused",
  );
  for (const r of requirements) {
    assert(
      r.network === "algorand:" + ALGO_GENESIS &&
        r.asset === "31566704" &&
        r.scheme === "exact",
      "mainnet_terms_mismatch",
    );
    assert(
      atomic(r.amount) > 0n &&
  ...
  const txs = raw.map((bytes) => {
    ...
    assert(
      Buffer.from(encodeTransactionRaw(t)).equals(Buffer.from(bytes)),
  ...
  for (const t of txs) {
    assert(
      Buffer.from(t.genesisHash ?? []).toString("base64") === ALGO_GENESIS &&
        (!t.genesisId || t.genesisId === "mainnet-v1.0"),
    ...
    assert(
      !t.rekeyTo &&
        ...
        (t.note?.length ?? 0) <= 80 &&
        t.group?.length === 32,
      "algorand_batch_side_effect_refused",
    );
    assert(
      t.firstValid > 0n &&
  ...
  assert(
    f.type === "pay" &&
      ...
      (f.fee ?? 0n) <= maxSponsorFeeMicroAlgo,
    "algorand_batch_fee_refused",
  );
  requirements.forEach((r, i) => {
    ...
    assert(
      t.type === "axfer" &&
        ...
        !t.assetTransfer.assetSender,
      "algorand_batch_transfer_refused",
    );
  });
  const expected = groupTransactions(
    txs.map((t) => new Transaction({ ...t, group: undefined })),
  );
  txs.forEach((t, i) =>
    assert(
      Buffer.from(t.group!).equals(Buffer.from(expected[i]!.group!)),
      "algorand_batch_group_id_refused",
    ),
  );
