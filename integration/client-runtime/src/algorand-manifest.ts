...
import * as manifestProfile from "../../sdk/route-guard/batch-profiles/algorand-manifest.mjs";
const {
  validateAlgorandManifestProfile,
  validateAlgorandManifestLimits,
  validateAlgorandFeeQuote,
} = manifestProfile;

export type AlgorandManifestProfile =
...
const ATOMIC: AlgorandManifestProfile = "algorand-atomic-multi-item-v1";
...
export interface AlgorandManifestPlan {
  ...
  id: string;
  scope: string;
  group: ReturnType<typeof checkAlgorandBatchGroup>;
}
...
const key = (id: string, stage: string) =>
  digest(canonical(["algorand-manifest-v2", id, stage]));
const snapshot = <T>(value: T): T => JSON.parse(canonical(value));
...
const count = (
  profile: AlgorandManifestProfile,
  limits: AlgorandManifestLimits,
) => (profile === ATOMIC ? limits.job_hashes.length : 1);

export function quoteAlgorandManifestFees(
  params: any,
  paymentCount: number,
  now = nowSeconds(),
  lifetime = 45,
): AlgorandManifestFeeQuote {
  assert(
    Number.isSafeInteger(paymentCount) &&
      ...
      lifetime >= 1 &&
      lifetime <= 60,
    "fee_quote_scope_refused",
  );
  assert(
    params?.["genesis-hash"] === GENESIS &&
...
export function checkCurrentAlgorandManifestQuote(
  quote: AlgorandManifestFeeQuote,
  params: any,
  now = nowSeconds(),
) {
  ...
  assert(
    now >= quote.observedAt &&
      now < quote.expiresAt &&
      String(params["min-fee"]) === quote.minFeeMicroAlgo &&
      params.fee === 0 &&
      BigInt(params["last-round"]) >= BigInt(quote.firstValid) - 1n &&
      BigInt(params["last-round"]) <= BigInt(quote.lastValid),
    "fee_quote_stale_or_changed",
  );
}
export function createAlgorandManifestOffer(
  profile: AlgorandManifestProfile,
  url: string,
...
export function buildAlgorandManifestTransactions(
  profile: AlgorandManifestProfile,
  envelope: any,
  limits: AlgorandManifestLimits,
  buyer: string,
) {
  const manifest = validateAlgorandManifestProfile(
      envelope,
      context(envelope.resource?.url),
      limits,
      profile,
    ),
  ...
  const note = (i: number) =>
    Buffer.from(`s2:${digest(canonical(manifest))}:${i}`);
  const transactions = [
    new Transaction({
      ...shared,
...
export function prepareAlgorandManifest(input: {
  ...
  const { profile, buyer } = input,
    envelope = snapshot(input.envelope),
    limits = snapshot(input.limits);
  const manifest = validateAlgorandManifestProfile(
    envelope,
    context(envelope.resource?.url),
    limits,
  ...
  assert(
    Array.isArray(input.raw) && input.raw.length === manifest.paymentCount + 1,
    "manifest_group_size_refused",
  );
  const raw = input.raw.map((b) => new Uint8Array(b)),
    req = envelope.accepts[0];
  const group = checkAlgorandBatchGroup(
    raw,
    manifest.paymentIndices,
    Array.from({ length: manifest.paymentCount }, () => req),
    buyer,
    limits.max_total_amount_atomic,
    BigInt(limits.max_sponsor_fee_micro_algo),
  );
  const expected = buildAlgorandManifestTransactions(
    profile,
    envelope,
    limits,
    buyer,
  ...
  assert(
    raw.every((b, i) => Buffer.from(b).equals(Buffer.from(expected[i]!))),
    "manifest_unsigned_group_mismatch",
  );
  assert(
    new Set(raw.map((b) => decodeTransaction(b).txId())).size === raw.length,
    "manifest_duplicate_transaction",
  );
  ...
  assert(
    encode64(sizedPayment).length <= ALGORAND_MANIFEST_HEADER_MAX,
    "manifest_header_too_large",
  );
  const id = digest(canonical([profile, NETWORK, group.groupId])),
    scope = digest(
      canonical({ profile, envelope, limits, buyer, groupId: group.groupId }),
    );
  return { profile, envelope, limits, buyer, raw, manifest, id, scope, group };
}
export async function signAlgorandManifest(
  input: AlgorandManifestPlan,
  sign: ManifestSigner,
  now = nowSeconds(),
) {
  const p = prepareAlgorandManifest(input),
    q = p.manifest.feeQuote;
  assert(now >= q.observedAt && now < q.expiresAt, "fee_quote_expired");
  const signed = await sign(
    p.raw.map((b) => new Uint8Array(b)),
    [...p.manifest.paymentIndices],
  );
  assert(
    Array.isArray(signed) && signed.length === p.raw.length && !signed[0],
    "manifest_signer_roles_refused",
  );
  const group = [Buffer.from(p.raw[0]!).toString("base64")];
  for (const i of p.manifest.paymentIndices) {
    const bytes = signed[i];
    assert(
      bytes instanceof Uint8Array && bytes.length <= 4096,
      "manifest_signature_refused",
    );
    const s = decodeSignedTransaction(bytes);
    assert(
      Buffer.from(encodeTransactionRaw(s.txn)).equals(Buffer.from(p.raw[i]!)) &&
        Buffer.from(encodeSignedTransaction(s)).equals(Buffer.from(bytes)) &&
        s.sig?.length === 64 &&
        !s.msig &&
        !s.lsig &&
        !s.authAddress,
      "manifest_signed_group_mismatch",
    );
    assert(
      await ed25519Verifier(
        s.sig!,
        bytesForSigning.transaction(s.txn),
        Address.fromString(p.buyer).publicKey,
      ),
      "manifest_signature_refused",
    );
  ...
  assert(
    encode64(payment).length <= ALGORAND_MANIFEST_HEADER_MAX,
    "manifest_header_too_large",
  );
  return payment;
}
export async function checkSignedAlgorandManifest(
  profile: AlgorandManifestProfile,
  envelope: any,
  limits: AlgorandManifestLimits,
  buyer: string,
  ...
  assert(
    canonical(payment.accepted) === canonical(envelope.accepts[0]) &&
  ...
  assert(
    data &&
      ...
      data.paymentGroup.length === count(profile, limits) + 1,
    "manifest_payment_shape_refused",
  );
  const raw = data.paymentGroup.map((s: unknown, i: number) => {
  ...
  const reproduced = await signAlgorandManifest(
    plan,
    async () =>
      data.paymentGroup.map((s: string, i: number) =>
        i ? decode64(s, 4096) : undefined,
      ),
    plan.manifest.feeQuote.observedAt,
  );
  assert(
    canonical(reproduced) === canonical(payment),
    "manifest_signed_group_mismatch",
  );
  return plan;
}
export function validateAlgorandManifestReceipt(
  ...
  assert(
    out.status === 200 &&
      ...
      b.batch.items.length === plan.manifest.jobCount,
    "manifest_receipt_unknown",
  );
  for (let i = 0; i < b.batch.items.length; i++) {
    const item = b.batch.items[i],
      paymentIndex = plan.profile === ATOMIC ? i + 1 : 1;
    assert(
      item.index === i + 1 &&
...
export async function executeAlgorandManifest(
  store: AlgorandManifestJournal,
  operationId: string,
  input: AlgorandManifestPlan,
  options: {
  ...
  assert(
    ...
    "manifest_operation_id_refused",
  );
  const p = prepareAlgorandManifest(input),
    clock = options.now ?? nowSeconds;
  const binding = {
    scope: p.scope,
  ...
  if (!store.once(key(operationId, "attempt"), binding)) {
    assert(
      store.get(key(operationId, "attempt"))?.scope === p.scope,
  ...
  try {
    assert(
      store.once(key(p.id, "plan-authority"), { operationId, scope: p.scope }),
      "manifest_authority_already_claimed",
    );
    await options.authorize(prepareAlgorandManifest(p));
    checkCurrentAlgorandManifestQuote(
      p.manifest.feeQuote,
      await options.readParams(),
      clock(),
    );
    ...
    assert(
      store.once(key(operationId, "send-permit"), { scope: p.scope }),
      "manifest_send_already_claimed",
    );
...
export async function recoverAlgorandManifest(
  store: AlgorandManifestJournal,
  operationId: string,
  input: AlgorandManifestPlan,
  read: (
    request: ManifestRecoveryRequest,
  ) => Promise<ManifestOutcome & { recoveryOnly: true }>,
) {
  try {
    const p = prepareAlgorandManifest(input),
      binding = store.get(key(operationId, "attempt"));
    assert(binding?.scope === p.scope, "manifest_scope_conflict");
    const saved = store.get(key(operationId, "outcome"));
    if (saved) return validateAlgorandManifestReceipt(p, saved);
    assert(
      store.get(key(operationId, "send-permit"))?.scope === p.scope,
      "manifest_never_submitted",
    );
    const payment = store.get(key(operationId, "credential"));
    await checkSignedAlgorandManifest(
      p.profile,
      p.envelope,
      p.limits,
      p.buyer,
    ...
    assert(
      ...
      "manifest_recovery_contract_required",
    );
...
export async function confirmAlgorandManifestOnce(
  input: AlgorandManifestPlan,
  rpcUrl: string,
  read: ManifestRead,
) {
  try {
    ...
    for (const raw of p.raw) {
      const expected = decodeTransaction(raw),
        id = expected.txId(),
        found = await read(endpoint + "/v2/transactions/pending/" + id),
        n = found.body?.["confirmed-round"];
      assert(
        found.status === 200 &&
          Number.isSafeInteger(n) &&
          n > 0 &&
          !found.body?.["pool-error"],
        "group_unconfirmed",
      );
      const actual = transactionCodec.decode(found.body?.txn?.txn, "json");
      assert(
        Buffer.from(encodeTransactionRaw(actual)).equals(Buffer.from(raw)) &&
          actual.txId() === id,
        "group_effect_mismatch",
      );
      if (round === undefined) round = n;
      else assert(round === n, "group_round_mismatch");
