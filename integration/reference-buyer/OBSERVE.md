...
Obtaining code and installing dependencies are separate network operations. The last published GitHub archive is `402signal-route-guard-0.7.1.tgz`. Verify that digest in `/capabilities.json` before installing. route-guard 0.7.2 is pending a GitHub release; do not treat a local `npm pack` digest as published. Checksum files are named `SHA256SUMS`, not `SHA256SUMS.txt`. See `/capabilities.json` for exact download URLs. A GitHub archive is not an npm-registry release. This source example uses the checkout's guard through its existing locked local dependency:
