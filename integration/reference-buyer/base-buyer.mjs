...
export class BaseBuyer {
  ...
  async #sign(id, stage, challenge, expected, assertFresh = () => {}) {
  ...
  async executeSellerOnce(
    ...
    await withVerifiedRoute(
      ...
      async (action) => {
        // The signed observation must still be current at each authority boundary.
        let previous = verifiedAt;
        const assertFresh = () => {
          const current = this.#now();
          check(Number.isSafeInteger(current) && current >= previous && current < action.expires_at, "expired_route_evidence");
          previous = current;
        };
        assertFresh();
        check(
        ...
        const value = await this.#sign(id, "seller", c, seller, assertFresh);
        assertFresh();
