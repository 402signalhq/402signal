...
import { verifyReceipt, verifyRoute } from "@402signal/route-guard";
...
for (const delay of ["chain_rpc", "balance_rpc", "factory_entry", "signer_entry", "signer_return", "factory_return", "before_send", "clock_backwards"]) {
  test("seller evidence freshness is enforced across " + delay, async () => {
    let clock = fixture.now, armed = false, s;
    const evidence = verifyRoute({
      routeResponseJson: JSON.stringify(fixture.response), routeRequestJson: JSON.stringify(fixture.request),
      trustedLogVkey: fixture.trusted_vkey, request: {url: fixture.url, method: "GET", body: new Uint8Array()},
      challenge: wire(fixture.challenge), now: clock,
    });
    const advance = () => { clock = delay === "clock_backwards" ? fixture.now - 1 : evidence.expires_at; };
    Date.now = () => clock * 1000;
    s = setup({buyerOptions: {
      now: () => clock,
      rpc: async (method, args) => {
        const value = await s.rpc(method, args);
        if (armed && ((delay === "chain_rpc" || delay === "clock_backwards") && method === "eth_chainId" || delay === "balance_rpc" && method === "eth_call")) advance();
        return value;
      },
      createSellerPayload: async ({challenge, account}) => {
        if (delay === "factory_entry") advance();
        const payload = await sdkPayload({challenge, account});
        if (delay === "factory_return") advance();
        return payload;
      },
    }});
    try {
      await routed(s); armed = true;
      const originalSign = s.account.signTypedData.bind(s.account);
      s.account.signTypedData = async data => {
        const signature = await originalSign(data);
        if (delay === "signer_return") advance();
        return signature;
      };
      const originalPut = s.journal.put.bind(s.journal);
      s.journal.put = (id, part, value) => {
        const result = originalPut(id, part, value);
        if (delay === "before_send" && part === "seller_submission" || delay === "signer_entry" && part === "seller_typed_data") advance();
        return result;
      };
      await assert.rejects(s.buyer.executeSellerOnce("job", {
        outcome:s.outcome, routeRequestJson:JSON.stringify(fixture.request),
        trustedLogVkey:fixture.trusted_vkey, challenge:wire(fixture.challenge),
      }), /expired_route_evidence/);
      assert.equal(s.counts().paid, 0);
      if (["chain_rpc", "balance_rpc", "factory_entry", "signer_entry", "clock_backwards"].includes(delay)) assert.equal(s.counts().signatures, 1);
      else assert.equal(s.counts().signatures, 2);
      // A claimed authorization remains claimed; no automatic replacement payment is made.
    ...
    } finally {s.close(); Date.now = () => fixture.now * 1000;}
  });
}
