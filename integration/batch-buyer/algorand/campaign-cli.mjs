...
const fail = () => {
  throw new Error(
    "Usage: campaign-cli.mjs plan POLICY | run|recover POLICY PRIVATE_JOURNAL JOB OWNER_HOOKS_MODULE",
  );
};
...
try {
  ...
  if (command === "plan") {
    ...
    const hooks =
      command === "run"
        ? await module.createRunHooks(policy)
        : await module.createReadOnlyRecoveryHooks(policy);
    console.log(JSON.stringify(await campaign[command](id, hooks), null, 2));
  }
} catch (error) {
  console.error(error instanceof Error ? error.message : "campaign_failed");
  process.exitCode = 1;
} finally {
  campaign?.close();
}
