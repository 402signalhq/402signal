...
// A response double with a fresh, metered Node-style body for each read.
...
test("fetchChallenge meters Node readable bodies and refuses an unmetered transport (S2 refresh)", async () => {
  ...
  assert.equal(read, 0);
  // Content-Length can describe compressed bytes or be false. It cannot make text() bounded.
  const declares = (length, encoding = null) => async () => ({
    status: 402,
    headers: { get: (n) => n === "content-length" ? String(length) : n === "content-encoding" ? encoding : null },
    text: async () => { read += 1; throw new Error("unbounded read must never start"); },
  });
  for (const length of [0, 10, body.length, 64 * 1024 + 1]) {
    for (const encoding of [null, "gzip", "br"]) {
      await assert.rejects(() => fetchChallenge(declares(length, encoding), "https://seller.example/x402"), /challenge_unbounded_transport/);
    }
  }
  assert.equal(read, 0);
