...
test("metadata and declared resource mutations remain bound before callback",()=>{
 for(const change of [{serviceName:"other"},{tags:[...f.challenge.resource.tags].reverse()},{tags:["other"]},{url:f.response.url},{url:f.response.url.split("?")[0]+"?query=other"},{url:"https://other.example/search"},{iconUrl:"https://example.com/icon"}]){const o=options(),env=structuredClone(f.challenge);Object.assign(env.resource,change);o.challenge.bodyText=JSON.stringify(env);reject(o);}
});
test("typed bounded metadata rejects before quote comparison",()=>{
 for(const change of [{serviceName:null},{serviceName:{}},{serviceName:""},{serviceName:"x".repeat(33)},{serviceName:"x\n"},{serviceName:"café"},{tags:"x"},{tags:[null]},{tags:Array(17).fill("x")},{tags:["x".repeat(33)]},{tags:[""]},{tags:["x\n"]},{tags:[{}]},{iconUrl:null},{iconUrl:""},{iconUrl:"http://example.com/icon"},{iconUrl:"https://u:p@example.com/icon"},{iconUrl:"https://example.com/icon#x"},{iconUrl:"https://example.com/"+"a".repeat(2048)}]){const o=options(),env=structuredClone(f.challenge);Object.assign(env.resource,change);o.challenge.bodyText=JSON.stringify(env);reject(o,"unsupported_resource");}
});
