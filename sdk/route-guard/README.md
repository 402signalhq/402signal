...
- `fetchChallenge` meters Node readable bodies (node-fetch style adapters)
  with the same 64 KiB loop as Web streams. Transports without either readable
  stream fail closed with `challenge_unbounded_transport` before any `text()`
  read. `Content-Length` cannot bound a decoded body, including compressed
  responses. Custom adapters must expose a readable stream.
