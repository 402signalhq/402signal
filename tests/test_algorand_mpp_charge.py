...
class NativeChargeBillingTests(unittest.TestCase):
    ...
    def test_valid_charge_bills_exact_router_fee_once_and_replays_after_memory_restart(self):
        for source in [VECTORS[0],VECTORS[4]]:
            ...
            b=bb.build(v["request"],observation(v));from live402 import batch_codec;result={"url":v["request"]["url"],**batch_codec.identity("mpp"),"live":True,"payable":True,"invocable":False,"status":402,"selected_payment":None,"batch_terms":b["terms"],"_batch_observation":observation(v)}
            with patch.dict(os.environ,{"BATCH_OBSERVATION_PROFILES":"mpp"}),patch("live402.route.run_probe",return_value=(200,result)),patch("live402.facilitator.verify",return_value=_verified())as verify,patch("live402.facilitator.settle",return_value=_settled())as settle:
                req=old.buyer(v["request"]);out=route.handle_route(req,_headers(_payload()),RESOURCE);self.assertEqual(out[1]["billing"]["amount_atomic"],"3000");self.assertTrue(out[1]["billing"]["settled"],out);replay.reset_memory();self.assertEqual(route.handle_route(req,_headers(_payload()),RESOURCE),out);self.assertEqual((verify.call_count,settle.call_count),(1,1))
