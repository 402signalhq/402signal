    ...
    def test_hosted_tool_guidance_survives_both_protocols_and_stdio(self):
        ...
        for version in ("2025-03-26", "2025-06-18"):
            with self.subTest(version=version):
                remote = json.loads((fixtures / f"tools-{version}.json").read_text(encoding="utf-8"))
                def opener(http_request, timeout):
                    self.assertEqual(json.loads(http_request.data), request)
                    self.assertEqual(http_request.get_header("Mcp-protocol-version"), version)
                    return io.BytesIO(json.dumps(remote).encode())

                out = io.StringIO()
                adapter.serve(io.BytesIO((json.dumps(request) + "\n").encode()), out,
                              lambda message: adapter.forward(message, opener, protocol_version=version))
                result = json.loads(out.getvalue())
                self.assertEqual(result, remote)
                actual = result["result"]["tools"]
                self.assertEqual([tool["name"] for tool in actual], ["check", "preview", "validate"])
                for tool in actual:
                    self.assertIn("description", tool)
                    self.assertIn("inputSchema", tool)
                    self.assertIn("annotations", tool)
                    self.assertEqual("outputSchema" in tool, version == "2025-06-18")
