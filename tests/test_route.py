...
class PaywallTests(unittest.TestCase):
    ...
    def test_homepage(self):
        ...
        self.assertIn("$0.003 per qualifying hosted check.", html)
        self.assertIn("Completed no-match checks are free", html)
        ...
        self.assertIn("Check x402 and MPP compatibility", html)
        self.assertIn("your local guard verifies a signed record before your wallet signs.", html)
        ...
        self.assertIn('query.get("need")', js)
        ...
        self.assertIn('$("need-chips").addEventListener("click"', js)
        css_path = os.path.join(os.path.dirname(__file__), "..", "live402", "static", "styles.css")
        with open(css_path, encoding="utf-8") as fh:
            css = fh.read()
        for swatch in ("#1c1c22", "#25252d", "#f7ebd4", "#c8b698", "#4a453c", "#fec865", "#e49c60", "#3ecfc9", "#17171d"):
        ...
        self.assertIn("<code>need</code> to find candidates", devs)
        self.assertIn("billing.settlement_state", devs)
        self.assertIn("unknown settlement", devs)
        self.assertIn("Do not reuse an unknown authorization", devs)
    ...
    def test_desc_under_cdp_500(self):
        ...
        self.assertIn("**$0.003 USDC**", readme)
