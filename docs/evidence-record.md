...
A versioned, protocol-independent description of the evidence 402Signal issues for one checked offer: the request, observed terms, submitted rules, recorded decision, time, signature and anchor. This page maps those parts to the receipts the service issues.

Short name: `offer_evidence_record_v1`. This describes the v4 exact x402 binding and the v5 group-offer observation, including named native MPP profiles. Agent-mandate references remain reserved. It introduces no new wire format, key, signer or on-chain transaction.

## What the record establishes

A record authenticates the terms 402Signal observed and the decision it recorded for a specific request. Verify it against an independently pinned log key, then compare it with your approved policy and wallet records. The receipt does not establish that a seller payment occurred or that the service was delivered.
...
Private, retained by the buyer: the request, the full response, the reveal, the observed challenge, the seller URL and covered request body. Public, in the log: the commitment, the leaf type, the rounded time, the nonce, the checkpoints and the anchors. The record does not contain the purchased service's output, wallet keys or executable buyer payment credentials. Original unpaid challenge channels remain part of the private evidence for the profiles that bind them. Bounded private response recovery exists for 120 seconds behind a client-held `Replay-Key`; it is not evidence storage.
...
| --- | --- | --- |
| `x402-exact-v2` | An x402 v2 `PaymentRequired` envelope with supported `exact` scheme options on Base, Solana and Algorand. | Issued today as the v4 binding. |
...
| `mpp-charge-v1` | A `WWW-Authenticate: Payment` challenge classified as a charge, with the method, intent, amount, recipient and network. | Named Base and Algorand charge profiles have v5 signed observations. Generic charge parsing alone remains observation-only. |
| `mpp-session-v1`, `mpp-subscription-v1` | Session and subscription terms: unit price, suggested deposit, period. | The named Solana push-session profile has a v5 observation. Generic session and subscription parsing remains observation-only. |
...
Each binding version defines its exact fields, commitment domain and verifier. Check [supported integrations](https://402signal.com/developers/supported-profiles) and the [current hosted capabilities](https://402signal.com/capabilities.json) before requesting a named profile.
...
Use `verifyReceipt` from `@402signal/route-guard`, the `signal402` Python verifier, or the [browser verifier](https://402signal.com/verify) for supported retained records. Live seller-offer comparison uses the JavaScript guard: `withVerifiedRoute` for exact x402 or the matching v5 guard for group-offer profiles. Historical verification does not authorize another payment or verify a later Falcon anchor.

## Versioning

`evidence_version` and the leaf type (`402signal.route_decision.v4`, `402signal.route_decision.v5`) version the commitment. Historical leaves keep their original verification semantics; retain a verifier that supports the record's version. A change to the commitment scheme or signature requires the corresponding versioned contract.

## Limits of the claim

The record authenticates what 402Signal recorded about a request, offer, time and decision. It does not prove settlement, delivery, output quality, seller identity or intent, legality, or that a human approved the submitted rules. It cannot recover a deleted private record and it cannot observe purchases that bypassed the check. The Falcon anchor protects the checkpoint history; it does not make the seller payment post-quantum secure.

Related: [proof-carrying route v1](proof-carrying-route-v1.md) (the v4 binding contract), [check group offer](batch-observation-v1.md) (v5), [route recovery](route-recovery.md), [investigating with retained evidence](customer/evidence.md).
