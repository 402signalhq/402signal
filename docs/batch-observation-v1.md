...
The feature defaults off. Operators enable codecs through the comma-separated `BATCH_OBSERVATION_PROFILES` setting. Tokens may be codecs (`exact`, `sess`, `mpp`, `atom`, `inv`) or legacy internal profile names, which map to those codecs. Empty enablement keeps the job off. An unpaid `POST /route` HTTP 402 is the router payment challenge. It is not proof that `buyer_limits` were admitted or that Check group offer validation passed. Unsupported or invalid_need body errors are returned only after successful verification, skip settlement, and refuse before the seller probe.
