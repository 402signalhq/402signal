...
Check [current hosted capabilities](https://402signal.com/capabilities.json) for enabled codecs and named profiles. An unpaid `POST /route` HTTP 402 is the router payment challenge. It is not proof that `buyer_limits` were admitted or that Check group offer validation passed. Unsupported or invalid_need body errors are returned only after successful verification, skip settlement, and refuse before the seller probe.
...
Live customer requests omit `merchant_profile`; explicit historical fixture profiles remain supported by offline verification. Unknown keys, repeated JSON keys, unsupported policies, redirects, ambiguous challenge headers, and truncated raw responses fail closed.
...
Use `verifyBatchRoute` or `withVerifiedBatchRoute` from `@402signal/route-guard/batch` (v0.7.2 or later) with the original route request JSON, response JSON, independently pinned log verification key, and original challenge `{status, bodyText, paymentRequired, wwwAuthenticate}`. Missing channels are explicit null. The verifier checks the v5 domain, commitment, Merkle inclusion, Ed25519 checkpoint, exact request and raw challenge, all policy pins and expiry before returning the observation or invoking the callback. Customer requests omit `merchant_profile`; historical leaves that included it still verify. An observation is not a blanket signing or spending authorization.
