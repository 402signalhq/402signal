...
Treat malformed headers and verification exceptions as a refusal. Verify the raw body before parsing the event, and deduplicate accepted deliveries before acting on them.
...
Every call answers only for the key that created the subscription. There is no listing across keys and no way to read another key's subscriptions. To rotate the signing secret, delete and recreate the subscription.
